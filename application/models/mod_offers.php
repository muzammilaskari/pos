...
class mod_offers extends CI_Model {
	...
	public function get_all_offers(){
		...
		$all_offer = $get_all_users->result_array();
		
		return $all_offer;
	}
	public function get_all_offers_product(){
		$this->db->dbprefix('product');
		$this->db->select('*');
	  	$this->db->where('status','1');
		$this->db->where('product_type','1');
	...
	public function del_offer_record($id){
	...
	public function get_offer_detail_record($id){
		...
		$get_all_users = $this->db->get('offer_detail');
	...
	public function add_offir($data){
		extract($data);
		...
		//-------------- Offer Detail Inserted---------------
		for($i=0;$i<count($product_id);$i++){
			$ins_data1 = array(
			   'offer_id' => $this->db->escape_str(trim($offer_id)),
			   'product_id' => $this->db->escape_str(trim($product_id[$i])),
			   'product_quantity' => $this->db->escape_str(trim($quantity[$i])),
			   'created_date' => $this->db->escape_str(trim($created_date)),
			   'created_by' => $this->db->escape_str(trim($created_by)),
			   'created_by_ip' => $this->db->escape_str(trim($ip_address)),
			   'modify_date' => $this->db->escape_str(trim($created_date)),
			...
			//Insert the record into the database.
			$this->db->dbprefix('offer_detail');
			$ins_into_db = $this->db->insert('offer_detail', $ins_data1);
		}
